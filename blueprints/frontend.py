...
def login_required(func):
    ...
    async def wrapper(*args, **kwargs):
        if not session:
            return await flash('error', 'Debe iniciar sesión para acceder a esa página.', 'login')
...
async def settings_profile_post():
    ...
    if new_name is None or new_email is None:
        return await flash('error', 'Parámetros inválidos.', 'home')
    ...
    if (
        ...
        return await flash('error', 'No se han realizado cambios.', 'settings/profile')

    if new_name != old_name:
        if not session['user_data']['is_donator']:
            return await flash('error', 'Los cambios de nombre de usuario son por el momento una ventaja para los colaboradores.', 'settings/profile')
        ...
        if not regexes.username.match(new_name):
            return await flash('error', 'La sintaxis de su nuevo nombre de usuario no es valida.', 'settings/profile')

        if '_' in new_name and ' ' in new_name:
            return await flash('error', 'Su nombre de usuario puede contener "_" o " ", pero no ambos.', 'settings/profile')

        if new_name in glob.config.disallowed_names:
            return await flash('error', "Su nombre de usuario no está permitido, eliga otro.", 'settings/profile')

        if await glob.db.fetch('SELECT 1 FROM users WHERE name = %s', [new_name]):
            return await flash('error', 'Su nombre de usuario ya ha sido tomado por otro usuario.', 'settings/profile')
    ...
    if new_email != old_email:
        ...
        if not regexes.email.match(new_email):
            return await flash('error', 'La sintaxis de su nuevo correo electrónico no es valida.', 'settings/profile')

        if await glob.db.fetch('SELECT 1 FROM users WHERE email = %s', [new_email]):
            return await flash('error', 'Su nuevo correo electrónico ya ha sido tomado por otro usuario.', 'settings/profile')
    ...
    return await flash('success', 'Su nombre de usuario/correo electrónico ha sido cambiado. Por favor, inicie sesión de nuevo.', 'login')
...
async def settings_avatar_post():
    ...
    if avatar is None or not avatar.filename:
        return await flash('error', 'No se ha seleccionado ninguna imagen.', 'settings/avatar')

    filename, file_extension = os.path.splitext(avatar.filename.lower())

    # bad file extension; deny post
    if not file_extension in ALLOWED_EXTENSIONS:
        return await flash('error', 'La imagen que seleccione debe ser un archivo .JPG, .JPEG, o .PNG', 'settings/avatar')
    ...
    return await flash('success', 'Su avatar ha sido cambiado con éxito.', 'settings/avatar')
...
async def settings_custom_post():
    ...
    if banner is None and background is None:
        return await flash_with_customizations('error', 'No se ha seleccionado ninguna imagen.', 'settings/custom')

    if banner is not None and banner.filename:
        _, file_extension = os.path.splitext(banner.filename.lower())
        if not file_extension in ALLOWED_EXTENSIONS:
            return await flash_with_customizations('error', f'El banner que seleccione debe de ser un archivo .JPG, .JPEG, .PNG o .GIF', 'settings/custom')
    ...
    if background is not None and background.filename:
        ...
        if not file_extension in ALLOWED_EXTENSIONS:
            return await flash_with_customizations('error', f'El fondo de perfil que seleccione debe de ser un archivo .JPG, .JPEG, .PNG o .GIF', 'settings/custom')
    ...
    return await flash_with_customizations('success', 'Su customización ha sido cambiada con éxito.', 'settings/custom')
...
async def settings_password_post():
    ...
    if new_password != repeat_password:
        return await flash('error', "Sus nuevas claves no coinciden.", 'settings/password')

    # new password and old password match; deny post
    if old_password == new_password:
        return await flash('error', 'Su nueva clave no puede ser la misma que su antigua clave.', 'settings/password')
    ...
    if not 8 < len(new_password) <= 32:
        return await flash('error', 'Su nueva clave debe tener entre 8 y 32 caracteres.', 'settings/password')

    if len(set(new_password)) <= 3:
        return await flash('error', 'Su nueva clave debe tener más de 3 caracteres únicos.', 'settings/password')

    if new_password.lower() in glob.config.disallowed_passwords:
        return await flash('error', 'Su nueva clave fue considerada demasiado simple.', 'settings/password')
    ...
    if pw_bcrypt in bcrypt_cache:
        if pw_md5 != bcrypt_cache[pw_bcrypt]: # ~0.1ms
            ...
            return await flash('error', 'Su antigua clave es incorrecta.', 'settings/password')
    else: # ~200ms
    ...
    session.pop('authenticated', None)
    session.pop('user_data', None)
    return await flash('success', 'Su clave ha sido cambiada. Por favor, inicie sesión de nuevo.', 'login')
...
async def login():
    if 'authenticated' in session:
        return await flash('error', "Ya ha iniciado sesión.", 'home')
...
async def login_post():
    ...
    if not user_info or user_info['id'] == 1:
        ...
        return await flash('error', 'La cuenta no existe.', 'login')
    ...
    if pw_bcrypt in bcrypt_cache:
        if pw_md5 != bcrypt_cache[pw_bcrypt]: # ~0.1ms
            ...
            return await flash('error', 'La clave es incorrecta.', 'login')
    else: # ~200ms
        if not bcrypt.checkpw(pw_md5, pw_bcrypt):
            if glob.config.debug:
                log(f"{username}'s login failed - pw incorrect.", Ansi.LYELLOW)
    ...
    if not user_info['priv'] & Privileges.Normal:
        ...
        return await flash('error', 'Su cuenta está restringida. No se le permite iniciar sesión.', 'login')
    ...
    return await flash('success', f'¡Bienvenido de vuelta {username}!', 'home')
...
async def register():
    ...
    if not glob.config.registration:
        return await flash('error', 'Los registros se encuentran actualmente deshabilitados.', 'home')
...
async def register_post():
    ...
    if glob.config.hCaptcha_sitekey != 'changeme':
        ...
        if (
            ...
            return await flash('error', 'El captcha ha fallado.', 'register')
    ...
    if not regexes.username.match(username):
        return await flash('error', 'La sintaxis del nombre de usuario no es valida.', 'register')

    if '_' in username and ' ' in username:
        return await flash('error', 'El nombre de usuario puede contener "_" o " ", pero no ambos.', 'register')

    if username in glob.config.disallowed_names:
        return await flash('error', 'Nombre de usuario no permitido; eliga otro.', 'register')

    if await glob.db.fetch('SELECT 1 FROM users WHERE name = %s', username):
        return await flash('error', 'El nombre de usuario ya ha sido tomado por otro usuario.', 'register')

    # Emails must:
    # - match the regex `^[^@\s]{1,200}@[^@\s\.]{1,30}\.[^@\.\s]{1,24}$`
    # - not already be taken by another player
    if not regexes.email.match(email):
        return await flash('error', 'La sintaxis del correo electrónico no es valida.', 'register')

    if await glob.db.fetch('SELECT 1 FROM users WHERE email = %s', email):
        return await flash('error', 'El correo electrónico ya ha sido tomado por otro usuario.', 'register')
    ...
    if not 8 <= len(passwd_txt) <= 32:
        return await flash('error', 'La clave debe tener entre 8 y 32 caracteres.', 'register')

    if len(set(passwd_txt)) <= 3:
        return await flash('error', 'La clave debe tener más de 3 caracteres únicos.', 'register')

    if passwd_txt.lower() in glob.config.disallowed_passwords:
        return await flash('error', 'Esa clave fue considerada demasiado simple.', 'register')
...
async def logout():
    if 'authenticated' not in session:
        return await flash('error', "No puede cerrar sesión si no se encuentra logeado.", 'login')
    ...
    # render login
    return await flash('success', 'Se ha cerrado la sesión con éxito.', 'login')
...
# Beatmaps routes
@frontend.route('/bmp/<beatmap_id>')
@frontend.route('/bmpset')
@frontend.route('/bmpset/<beatmapset_id>')
@frontend.route('/bmpset/<beatmapset_id>/<mode>')
@frontend.route('/bmpset/<beatmapset_id>/<mode>/<beatmap_id>')
@frontend.route('/bmpset/<beatmapset_id>/<mode>/<beatmap_id>/<extra_mode>')
